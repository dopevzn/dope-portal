...
const documents = [
  {
    title: "Pitch Deck",
    description: "The complete 12-slide investor presentation covering the opportunity, traction, financial outlook, and capital plan.",
...
function AccessScreen({ error }: { error?: string }) {
  const message = error === "invalid" ? "That password is not correct. Please try again." : error === "configuration" ? "Secure access is being configured. Please contact Michael before retrying." : null;
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#050505] px-6 text-white">
      ...
      <div className="relative w-full max-w-lg rounded-[28px] border border-white/10 bg-black/80 p-7 shadow-2xl backdrop-blur-xl sm:p-10">
        ...
        <p className="mt-8 text-xs leading-5 text-white/30">Confidential materials for private discussion. Access is intended only for the named recipient.</p>
      </div>
    </main>
  );
}
...
function DavidRoom() {
  return (
    <main className="min-h-screen bg-[#050505] text-white">
      ...
      <section className="border-b border-white/10 bg-white/[0.02]"><div className="mx-auto grid max-w-7xl grid-cols-2 px-6 py-8 lg:grid-cols-4 lg:px-10">{[["$55K","Historical revenue","Dec. 2024-Dec. 2025"],["$10K","Investment discussion","Targeted growth capital"],["60%","Year 1 margin target","Base-case assumption"],["$225K","Year 5 revenue","Base-case forecast"]].map(([value,label,note])=><div key={label} className="border-white/10 px-3 py-6 lg:border-r lg:px-8 last:border-r-0"><p className="text-3xl font-black tracking-[-0.04em] sm:text-4xl">{value}</p><p className="mt-3 text-sm font-semibold text-white/80">{label}</p><p className="mt-1 text-xs text-white/35">{note}</p></div>)}</div></section>
      ...
      <footer className="border-t border-white/10"><div className="mx-auto flex max-w-7xl flex-col gap-4 px-6 py-8 text-xs text-white/30 sm:flex-row sm:items-center sm:justify-between lg:px-10"><p>DOPE VZN Capital Room · Prepared for David Keyser</p><p>Confidential · Private discussion only · Not a securities offering</p></div></footer>
    </main>
  );
}
